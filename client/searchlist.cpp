...
SearchList::SearchList(QWidget *parent):QListWidget(parent),_find_dlg(nullptr), _search_edit(nullptr), _send_pending(false)
{
    ...
    this->setStyleSheet(R"(
        ...
        #add_tip {
            border-image: url(:/image/res/addtip.png);
        }
        #right_tip {
            border-image: url(:/image/res/right_tip.png);
        }
        #message_tip {
            text-align: center;
...
{
    QWidget*widget=this->itemWidget(item); //获取widget
    if(!widget){
...
void SearchList::slot_user_search(std::shared_ptr<SearchInfo> si)
{
    ...
    if(si==nullptr){
        ...
        auto seld_uid=UserMgr::GetInstance()->GetUid();
        if(si->_uid==seld_uid)
            return;
