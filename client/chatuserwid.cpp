...
void ChatUserWid::SetInfo(std::shared_ptr<UserInfo> user_info)
{
    ...
    ui->user_chat_lb->setText(user_info->_last_msg);
}
...
void ChatUserWid::SetUSerInfo(std::shared_ptr<UserInfo> user_info)
{
    _user_info=user_info;
}
...
void ChatUserWid::SetInfo(std::shared_ptr<FriendInfo> user_info)
{
    _user_info=std::make_shared<UserInfo>(user_info);
    // 加载图片
    QPixmap pixmap(user_info->_icon);
